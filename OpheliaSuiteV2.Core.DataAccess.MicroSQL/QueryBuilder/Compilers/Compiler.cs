...
namespace OpheliaSuiteV2.Core.DataAccess.MicroSQL.QueryBuilder.Compilers {
    internal partial class Compiler {
        ...
        internal virtual string parameterPlaceholder { get; set; } = "?";
        internal virtual string parameterPlaceholderPrefix { get; set; } = "@p";
        internal virtual string OpeningIdentifier { get; set; } = "\"";
        internal virtual string ClosingIdentifier { get; set; } = "\"";
        internal virtual string ColumnAsKeyword { get; set; } = "AS ";
        internal virtual string TableAsKeyword { get; set; } = "AS ";
        internal virtual string LastId { get; set; } = "";
        ...
        private SqlResult CompileUpdateQuery(Query query) {
            ...
            if (toUpdate.Types.Count > 0) {
                for (var i = 0; i < toUpdate.Columns.Count; i++) {
                    parts.Add($"{Wrap(toUpdate.Columns[i])} = {{{toUpdate.ParameterNames[i]}}}");
                }
                setValues = string.Join(", ", parts);
            } else {
                for (var i = 0; i < toUpdate.Columns.Count; i++) {
        ...
        protected virtual SqlResult CompileInsertQuery(Query query) {
            ...
            if (inserts[0] is InsertClause insertClause) {
                ctx.RawSql = $"INSERT INTO {table}"
                    ...
                    + "VALUES (" + (insertClause.Types.Count > 0 ? this.Parameterize(ctx, insertClause.Columns, insertClause.Values, insertClause.Types, insertClause.ParameterNames) : Parameterize(ctx, insertClause.Values)) + ")";
            ...
            if (inserts.Count > 1) {
                foreach (var insert in inserts.GetRange(1, inserts.Count - 1)) {
                    ...
                    ctx.RawSql += ", (" + (clause.Types.Count > 0 ? this.Parameterize(ctx, clause.Columns, clause.Values, clause.Types, clause.ParameterNames) : Parameterize(ctx, clause.Values)) + ")";
        ...
        public virtual string CompileWheres(SqlResult ctx) {
            ...
            ctx.Parameters.AddRange(ctx.Query.Parameters.Where(p => !ctx.Parameters.Contains(p)));
        ...
        public virtual string Parameter(SqlResult ctx, string columnName, object value, Type type, string parName) {
            ctx.Bindings.Add(value);
            if (!ctx.Parameters.Any(p => p.Name == parName && p.ClrType.IsAssignableFrom(type)))
                ctx.Parameters.Add(new Parameter(parName, value, type));
            return parName;
        }

        public virtual string Parameterize(SqlResult ctx, IEnumerable<string> columns, IEnumerable<object> values, IEnumerable<Type> types, IEnumerable<string> parameterNames) {
            string parametes = string.Empty;
            for (int i = 0; i < columns.Count(); i++) {
                parametes += $",{{{Parameter(ctx, columns.ElementAt(i), values.ElementAt(i), types.ElementAt(i), parameterNames.ElementAt(i))}}}";
            }
            return parametes.Substring(1);
        }
